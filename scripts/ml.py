...
if __name__ == '__main__':
    ...
    ax.plot(UITRANS, np.arange(UNH), color='yellow')
    ax.plot(UICVAL, UICDOM, color='yellow', linestyle='--')
    ax.imshow(TSNEINIT[:, 0].reshape(UNH, UNT), aspect='equal', interpolation='none', origin='lower', cmap=CM)
    ax.grid(which='minor', axis='both', linestyle='-', color='k', linewidth=1)
    ax.set_xticks(np.arange(UT.size), minor=True)
    ax.set_yticks(np.arange(UH.size), minor=True)
    plt.xticks(np.arange(UT.size)[::4], np.round(UT, 2)[::4], rotation=-60)
    ...
    plt.ylabel('H')
    # plt.title('Ising Model Phase Diagram')
    fig.savefig(OUTPREF+'.vae.diag.pca0.png')
    ...
    ax.plot(UITRANS, np.arange(UNH), color='yellow')
    ax.plot(UICVAL, UICDOM, color='yellow', linestyle='--')
    ax.imshow(TSNEINIT[:, 1].reshape(UNH, UNT), aspect='equal', interpolation='none', origin='lower', cmap=CM)
    ax.grid(which='minor', axis='both', linestyle='-', color='k', linewidth=1)
    ax.set_xticks(np.arange(UT.size), minor=True)
    ax.set_yticks(np.arange(UH.size), minor=True)
    plt.xticks(np.arange(UT.size)[::4], np.round(UT, 2)[::4], rotation=-60)
    ...
    plt.ylabel('H')
    # plt.title('Ising Model Phase Diagram')
    fig.savefig(OUTPREF+'.vae.diag.pca1.png')
