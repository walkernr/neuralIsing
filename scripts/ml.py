...
if __name__ == '__main__':
    ...
    if PLOT:
        ...
        DIAGMLV = np.arctanh(2*np.mean(SCLRS['minmax'].fit_transform(SLPZENC.reshape(UNH*UNT*UNS, ED)).reshape(UNH, UNT, UNS, ED), 2)-1)
        ...
        DIAGSLV = np.arctanh(2*SCLRS['minmax'].fit_transform(np.var(SLPZENC/UT[np.newaxis, :, np.newaxis, np.newaxis], 2).reshape(UNH*UNT, ED)).reshape(UNH, UNT, ED)-1)
