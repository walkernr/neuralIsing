...
class InfoGAN():
    ...
    def fit(self, x_train, num_epochs=4, n_dsc=1, n_gan=2, save_step=None, random_sampling=False, verbose=False):
        ...
        if random_sampling:
            x_train = extract_unique_data(x_train, self.num_fields, self.num_temps, self.num_samples, self.input_shape)
        else:
            x_train = reorder_training_data(x_train, self.num_fields, self.num_temps, self.num_samples, self.input_shape, self.batch_size)
