...
class InfoCGAN():
    ...
    def train_generator(self, z_sample):
        ...
        if self.wasserstein:
            ...
            for i in range(len(z_sample)):
                ...
                gan_aux_loss = self.gan_aux.train_on_batch(z_sample[i], z_sample[i][2:])
            ...
            for i in range(len(z_sample)):
                gan_dsc_aux_loss = self.gan.train_on_batch(z_sample[i], (target, *z_sample[i][2:]))
                if self.c_dim > 0 and self.u_dim > 0:
    ...
    def train_infogan(self, x_batch, t_batch, n_dsc, n_gan):
        ...
        x_generated, t_generated = self.generator.predict(x=z_sample[0], batch_size=self.batch_size)
