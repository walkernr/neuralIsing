...
def plot_batch_losses(losses, cmap, file_prfx, verbose=False):
    ...
    # plot losses
    loss_list = ['VAE Loss', 'Latent Loss', 'Reconstruction Loss', 'Thermal Parameter Loss']
...
def plot_diagrams(m_data, s_data, fields, temps, cmap,
    ...
    if s_data is not None:
        ...
        if alias == 'z' or alias == 'z_p':
            d = 'Encodings'
        elif alias == 't':
            d = 'Thermal Params'
        for i in trange(zm_dim, desc='Plotting Mean VAE {}'.format(d), disable=not verbose):
            plot_diagram(zm_diag[:, :, i], fields, temps, cmap, file_prfx, '{}_m_{}'.format(alias, i))
        for i in trange(zs_dim, desc='Plotting StDv VAE {}'.format(d), disable=not verbose):
...
class VAE():
    ...
    def conditional_loss(self):
        err = (self.enc_t_input-self.t_output)*K.expand_dims(self.therm_range, axis=0)
        return K.sqrt(K.sum(K.square(err), axis=-1))
    ...
    def _build_encoder(self):
        ...
        if np.any(np.array([self.alpha, self.beta, self.lamb]) > 0):
            ...
            # build encoder
            self.encoder = Model(inputs=[self.enc_x_input, self.enc_t_input], outputs=[self.mu, self.logvar, self.z],
                                 name='encoder')
        else:
            # latent space
    ...
    def _build_decoder(self):
        ...
        # build decoder
        self.decoder = Model(inputs=[self.dec_z_input], outputs=[self.dec_x_output, self.dec_t_output],
                             name='decoder')
    ...
    def _build_vae(self):
        ...
        if np.all(np.array([self.alpha, self.beta, self.lamb]) == 0):
            self.x_output, self.t_output = self.decoder(self.encoder((self.enc_x_input, self.enc_t_input)))
            ...
            self.x_output, self.t_output = self.decoder(self.encoder((self.enc_x_input, self.enc_t_input))[2])
        ...
        cd_loss = self.conditional_loss()
        self.vae.add_loss(cd_loss)
        self.vae.add_metric(cd_loss, name='cd_loss', aggregation='mean')
    ...
    def get_losses(self):
        ...
        cd_loss = np.array(self.cd_loss_history).reshape(-1, self.num_batches)
        return vae_loss, tc_loss, rc_loss, cd_loss
    ...
    def load_losses(self, name, lattice_length, interval, num_samples, scaled, seed):
        ...
        self.cd_loss_history = list(losses[:, :, 3].reshape(-1))
    ...
    def train_vae(self, x_batch, t_batch, kl_anneal):
        ...
        # VAE losses
        if np.any(np.array([self.alpha, self.beta, self.lamb]) > 0):
            vae_loss, tc_loss, rc_loss, cd_loss = self.vae.train_on_batch([x_batch, t_batch, kl_anneal])
            self.vae_loss_history.append(vae_loss)
            self.tc_loss_history.append(tc_loss)
            self.rc_loss_history.append(rc_loss)
            self.cd_loss_history.append(cd_loss)
        else:
            vae_loss, rc_loss, cd_loss = self.vae.train_on_batch(x_batch, t_batch)
            self.vae_loss_history.append(vae_loss)
    ...
    def rolling_loss_average(self, epoch, batch):
        ...
        # calculate rolling average
            ...
            cd_loss = np.mean(self.cd_loss_history[start:stop])
        return vae_loss, tc_loss, rc_loss, cd_loss
    ...
    def fit(self, x_train, t_train, num_epochs=4, save_step=4, random_sampling=False, verbose=False):
        ...
        if random_sampling:
            ...
            x_train, t_train = reorder_training_data_thermal(x_train, t_train, self.num_fields, self.num_temps, self.num_samples, self.input_shape, self.t_dim)
